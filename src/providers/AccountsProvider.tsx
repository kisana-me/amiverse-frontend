"use client";

import {
  createContext,
  useContext,
  useState,
  useCallback,
  ReactNode,
  useRef,
  useEffect,
} from "react";
import { AccountType } from "@/types/account";
import { api } from "@/lib/axios";

export type CachedAccount = AccountType & {
  fetched_at: number;
};

type AccountsContextType = {
  accounts: Record<string, CachedAccount>;
  fetchAccount: (name_id: string, options?: { force?: boolean }) => Promise<AccountType | null>;
  getAccount: (name_id: string) => CachedAccount | undefined;
  updateAccount: (name_id: string, data: Partial<AccountType>) => void;
  seedAccount: (name_id: string, account: AccountType) => void;
};

const AccountsContext = createContext<AccountsContextType | null>(null);

export const AccountsProvider = ({ children }: { children: ReactNode }) => {
  const [accounts, setAccounts] = useState<Record<string, CachedAccount>>({});
  const accountsRef = useRef(accounts);

  useEffect(() => {
    accountsRef.current = accounts;
  }, [accounts]);

  const getAccount = useCallback((name_id: string) => {
    return accounts[name_id];
  }, [accounts]);

  const updateAccount = useCallback((name_id: string, data: Partial<AccountType>) => {
    setAccounts((prev) => {
      const current = prev[name_id];
      if (!current) return prev;
      return {
        ...prev,
        [name_id]: { ...current, ...data },
      };
    });
  }, []);

  const seedAccount = useCallback((name_id: string, account: AccountType) => {
    setAccounts((prev) => {
      if (prev[name_id]) return prev;
      return {
        ...prev,
        [name_id]: { ...account, fetched_at: Date.now() },
      };
    });
  }, []);

  const fetchAccount = useCallback(async (name_id: string, options?: { force?: boolean }) => {
    if (!options?.force && accountsRef.current[name_id]) {
      return accountsRef.current[name_id];
    }

    try {
      const res = await api.post('/accounts/@' + name_id);
      const account = res.data as AccountType;
      
      const cachedAccount: CachedAccount = {
        ...account,
        fetched_at: Date.now(),
      };

      setAccounts((prev) => ({
        ...prev,
        [name_id]: cachedAccount,
      }));

      return cachedAccount;
    } catch {
      return null;
    }
  }, []);

  const value: AccountsContextType = {
    accounts,
    fetchAccount,
    getAccount,
    updateAccount,
    seedAccount,
  };

  return (
    <AccountsContext.Provider value={value}>
      {children}
    </AccountsContext.Provider>
  );
};

export const useAccounts = () => {
  const context = useContext(AccountsContext);
  if (!context) {
    throw new Error("useAccounts must be used within an AccountsProvider");
  }
  return context;
};
